...
class ActionValueActorCriticAgent(Agent):
    """
    Simple actor-critic agent for discrete action spaces.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.actor_critic = MultiHeadedMLP(
            ...
            head_sizes=(self.num_actions, self.num_actions),
            activation=activation,
            head_activations=(nn.Softmax(dim=-1), None),
        )
    ...
    def evaluate_states(self, states):
        """
        Given environment states, return the log likelihoods of all actions under the current policy and an estimate of the state-action values.
        ...
        log_probs : torch.Tensor
            Log probability of all actions under the policy pi(a_t|s_t).

        values : torch.Tensor
            Estimated state-action values q(s_t, a_t) for all actions.
...
class ACERAgent(ActionValueActorCriticAgent):
